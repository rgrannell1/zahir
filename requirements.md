...
Per-function job counts (total, completed, failed) are bookman aggregators in the metrics module. The progress bar state model applies them incrementally as events arrive.

ProgressBarService uses a single per_fn_progress_agg() aggregator for both job counts and mean durations. TimeEstimator is removed; ETA computation lives in ProgressBarService using in-flight counts derived from the aggregator state.

Job evaluation is three composable generator layers: an invalid-effect guard innermost (throws InvalidEffect for non-Effect values, coordination effects, and blocked tertius effects), a handle() call above it dispatching job-level effects (EAcquire, EGetSemaphore, ESetSemaphore), and a timeout guard outermost injecting JobTimeout when the deadline passes. EAwait and coordination effects bubble through all three to the worker level.
