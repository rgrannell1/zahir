...
def test_wrap_runs_setup_before_handler():
    ...
    drain_to(wrapped(EAwait(jobs=[JobSpec("job")])))
...
def test_wrap_preserves_handler_return_value():
    ...
    _, value = drain_to(wrapped(EAwait(jobs=[JobSpec("job")])))
    assert value == 42
...
def test_wrap_propagates_teardown_yields():
    ...
    wrapped = wrap(f)(_make_handler(None))
    effects, _ = drain_to(wrapped(EAwait(jobs=[JobSpec("job")])))
...
def test_wrap_exposes_fn_via_args():
    """Proves wrap() stores fn as args[0] on the returned partial."""
