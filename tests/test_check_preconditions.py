...
def test_check_preconditions_pass_normal_job():
    ...
    job_id = context.job_registry.add(context, job, output_queue, "test-workflow")
...
def test_check_preconditions_timeout_normal_job():
    ...
    job_id = context.job_registry.add(context, job_instance, output_queue, "test-workflow")

    # Set job to running state first and record start time
    context.job_registry.set_state(
        context, job_instance.job_id, job_instance.spec.type, workflow_id, output_queue, JobState.RUNNING
    )
...
def test_check_preconditions_timeout_recovery_job():
    ...
    context.job_registry.set_state(
        context, job_instance.job_id, job_instance.spec.type, workflow_id, output_queue, JobState.RECOVERING
    )
