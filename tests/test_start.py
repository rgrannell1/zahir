...
def test_start_no_frame_with_paused_job_enqueues():
    ...
    required_job_id = context.job_registry.add(context, another_job, output_queue, workflow_id)
    context.job_registry.set_state(
        context, required_job_id, another_job.spec.type, workflow_id, output_queue, JobState.RUNNING
    )
