...
def test_handle_call_get_job_returns_job():
    ...
    state = _make_state()
    _, job = _handle_call(state, (OM.GET_JOB, b"worker-pid"))
    assert job[1] == "start"
...
def test_handle_call_acquire_grants_slot():
    ...
    state = _make_state()
    _, result = _handle_call(state, (OM.ACQUIRE, "workers", 4))
...
def test_handle_call_signal_returns_none_for_unknown_semaphore():
    ...
    _, result = _handle_call(_make_state(), (OM.SIGNAL, "db"))
...
def test_handle_call_is_done_false_initially():
    ...
    _, result = _handle_call(_make_state(), OM.IS_DONE)
...
def test_handle_cast_enqueue_adds_job():
    ...
    state = _handle_cast(_make_state(), (OM.ENQUEUE, "child", (42,), None, None))
...
def test_handle_cast_job_done_decrements_pending():
    ...
    state = _handle_cast(_make_state(), (OM.JOB_DONE, None, None, "result"))
...
def test_handle_cast_release_decrements_slot():
    ...
    state = _handle_cast(state, (OM.RELEASE, "workers"))
...
def test_handle_cast_set_semaphore_stores_state():
    ...
    state = _handle_cast(_make_state(), (OM.SET_SEMAPHORE, "db", "impossible"))
...
def test_enqueue_then_get_job_round_trips():
    ...
    state = _make_state()
    state = _handle_cast(state, (OM.ENQUEUE, "child", (1,), None, 1000))
    state, _ = _handle_call(state, (OM.GET_JOB, b"worker-pid"))  # dequeue initial job
    _, job = _handle_call(state, (OM.GET_JOB, b"worker-pid"))
    assert job[1] == "child"
    assert job[4] == 1000
...
def test_is_done_true_after_all_jobs_complete():
    ...
    state = _make_state()
    state, _ = _handle_call(state, (OM.GET_JOB, b"worker-pid"))
    state = _handle_cast(state, (OM.JOB_DONE, None, None, "result"))
    _, result = _handle_call(state, OM.IS_DONE)
    assert result is True
