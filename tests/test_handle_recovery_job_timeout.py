"""Tests for the handle_recovery_job_timeout state machine step.

Tests the handle_recovery_job_timeout function which handles when a recovery
job exceeds its timeout limit.
"""
...
def test_handle_recovery_job_timeout_returns_enqueue_state_change():
    """Test that handle_recovery_job_timeout returns EnqueueJobStateChange."""
    ...
    result, _ = handle_recovery_job_timeout(worker_state)
...
def test_handle_recovery_job_timeout_sets_recovery_timed_out_state():
    """Test that the job state is set to RECOVERY_TIMED_OUT."""
    ...
    assert job_state == JobState.RECOVERY_TIMED_OUT


def test_handle_recovery_job_timeout_records_error():
    """Test that a JobRecoveryTimeoutError is recorded."""
...
def test_handle_recovery_job_timeout_clears_frame():
...
def test_handle_recovery_job_timeout_preserves_state():
    """Test that handle_recovery_job_timeout returns the same state object."""
    ...
    # Run handle_recovery_job_timeout
    result, returned_state = handle_recovery_job_timeout(worker_state)

    # Verify same state object returned
    assert returned_state is worker_state


def test_handle_recovery_job_timeout_includes_job_type_in_message():
...
def test_handle_recovery_job_timeout_transitions_to_enqueue():
...
def test_handle_recovery_job_timeout_error_message():
    ...
    assert "Recovery job execution timed out" == error_msg
